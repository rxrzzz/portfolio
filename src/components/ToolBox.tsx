...
export const Toolbox = ({ name, imgLinks, description }: ToolboxProps) => {
  return (
    <article className=" flex flex-wrap xl:w-11/12  w-full border-2 py-8  font-author justify-evenly items-center xl:mb-16 mb-8 rounded-lg ">
      ...
      <div className="xl:gap-12 w-11/12  lg:[&>img]:mr-0 [&>img]:mr-5 xl:w-3/12  xl:grid xl:grid-cols-2 flex ">
        {imgLinks.map((img) => (
          <Image src={img} width={60} height={60} alt={name} key={img} className="xl:mx-5 border p-1 shadow-lg" title={img.replace("/", "").replace(".svg", "")}/>
        ))}
      </div>
    </article>
  );
